...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleError { error_type }) => panic!("{}", error_type),
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError {error_type: "Title cannot be empty".to_string()});
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {error_type: "Title cannot be longer than 50 bytes".to_string()});
        }
