...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        if title == "" {panic!("Title cannot be empty")}
        if title.len() > 50 {panic!("Title cannot be longer than 50 bytes")}
        if description == "" {panic!("Description cannot be empty")}
        if description.len() > 500 {panic!("Description cannot be longer than 500 bytes")}
        let allowed_status = ["To-Do", "In Progress", "Done"];
        if !allowed_status.contains(&status.as_str()) {panic!("Only `To-Do`, `In Progress`, and `Done` statuses are allowed")}
        Self {
            title,
            description,
            status,
